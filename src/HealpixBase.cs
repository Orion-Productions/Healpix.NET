...
namespace Healpix
{
    ...
    */
    ...
    public class HealpixBase : HealpixTables
    {
        protected class Xyf
        {
        ...
        private class RingInfoSmall
        {
            public long startpix, ringpix;
            public bool shifted;
        }

        /** Maximum order parameter */
        public static int order_max=29;
        /** Maximum Nside parameter; equivalent to 2^{@code order_max}. */
        public static long ns_max=1L<<order_max;
        ...
        private long xyf2ring(int ix, int iy, int face_num)
        {
            ...
            long nr = (long)((ulong)ris.ringpix>>2), kshift=ris.shifted ? 0:1;
        ...
        protected Xyf ring2xyf(long pix)
        {
            ...
            {
            iring = (long)((ulong)(1+HealpixUtils.isqrt(1L+2L*pix))>>1); //counted from North pole
            ...
            {
            long ip = pix - ncap;
            long tmp = (order>=0) ? (long)(((ulong)ip)>>(order+2)) : ip/nl4;
            iring = tmp+nside;
            iphi = ip-tmp*nl4 + 1;
            ...
            long ire = iring-nside+1,
                irm = nl2+2-ire;
            long ifm = iphi - (long)(((ulong)ire)>>1) + nside -1,
                ifp = iphi - (long)(((ulong)irm)>>1) + nside -1;
            if (order>=0)
                { ifm = (long)((ulong)ifm >> order); ifp = (long)((ulong)ifp >> order); }
            else
                { ifm /= nside; ifp /= nside; }
            ...
            {
            long ip = npix - pix;
            iring = (long)((ulong)(1+HealpixUtils.isqrt(2L*ip-1L))>>1); //counted from South pole
            ...
            ret.ix = (int)((ulong)( ipt-irt)>>1);
            ret.iy = (int)((ulong)(-ipt-irt)>>1);
        ...
        public HealpixBase(long nside_in, Scheme scheme_in)
        ...
        protected long loc2pix (Hploc loc)
        {
            ...
            if (scheme==Scheme.RING)
            {
                if (za<=Constants.twothird) // Equatorial region
                {
                    ...
                    long ip = (order>0) ?
                    (long)(((ulong)t1)>>1)&(nl4-1) : ((long)(((ulong)t1)>>1)%nl4); // in {0,4n-1}
                ...
                else  // North & South polar caps
                {
                    ...
                    double tmp = ((za<0.99)||(!loc.have_sth)) ?
                                ...
                                nside*loc.sth/Math.Sqrt((1.0+za)/3.0);
                    ...
                    System.Diagnostics.Debug.Assert((ip>=0)&&(ip<4*ir));
            ...
            else // scheme_ == NEST
            {
                if (za<=Constants.twothird) // Equatorial region
                {
                    ...
                    long jp = (long)(temp1-temp2); // index of  ascending edge line
                    long jm = (long)(temp1+temp2); // index of descending edge line
                    long ifp = (long)((ulong)jp >> order);  // in {0,4}
                    long ifm = (long)((ulong)jm >> order);
        ...
        protected Hploc pix2loc (long pix)
        {
            ...
            if (scheme==Scheme.RING)
            {
                if (pix<ncap) // North Polar cap
                {
                    long iring = (long)((ulong)(1+(HealpixUtils.isqrt(1+2*pix)))>>1); //from North pole
                    long iphi  = (pix+1) - 2*iring*(iring-1);

                    double tmp = (iring*iring)*fact2;
                    loc.z = 1.0 - tmp;
                    if (loc.z>0.99) { loc.sth=Math.Sqrt(tmp*(2.0-tmp)); loc.have_sth=true; }
                ...
                {
                    long ip = npix - pix;
                    long iring = (long)((ulong)(1+HealpixUtils.isqrt(2*ip-1))>>1); //from South pole
            ...
            else
            {
                ...
                {
                    nr = jr;
                    double tmp2 = (nr*nr)*fact2;
                    loc.z = 1 - tmp2;
                    if (loc.z>0.99) { loc.sth=Math.Sqrt(tmp2*(2.0-tmp2)); loc.have_sth=true; }
                }
                else if (jr>nl3)
                {
                    nr = nl4-jr;
                ...
                System.Diagnostics.Debug.Assert(tmp<8*nr); // must not happen
                if (tmp<0) tmp+=8*nr;
        ...
        public long[] neighbours(long ipix)
        {
            ...
            else
            {
                for (int i=0; i<8; ++i)
                {
                    ...
                    if (f>=0)
                    {
                        int bits = swaparray[nbnum, (ulong)face_num>>2];
        ...
        {
            Vec3 va = new Vec3(new Zphi(2.0/3.0, Math.PI/nl4));
        ...
        public double ring2theta (long ring)
        {
            if (ring<nside)
            {
                double tmp2=ring*ring*fact2;
                return FastMath.atan2 (Math.Sqrt(tmp2*(2.0-tmp2)),1-tmp2);
            }
            if (ring <=nl3)
            {
            ...
            return FastMath.atan2 (Math.Sqrt(tmp*(2.0-tmp)),tmp-1);
        }

        private class pstack
        {
            private long[] p;
            private int[] o;
            private int s, m;
        ...
        private RangeSet queryStripInternal(double theta1, double theta2, bool inclusive)
        {
        ...
        private bool checkPixelRing (HealpixBase b2, long pix, long nr,
        ...
        private RangeSet queryMultiDisc (Vec3[] norm, double[] rad, int fact)
        {
            bool inclusive = (fact!=0);
            int nv=norm.Length;
            ...
            if (scheme==Scheme.RING)
            {
                ...
                for (long iz=irmin; iz<=irmax; ++iz)
                {
                    ...
                    for (int j=0; (j<nd)&&(rstmp.nranges()>0); ++j)
                    {
                        ...
                        long ip_lo = (long)Math.Floor(nr*Constants.inv_twopi*(ptg[j].phi-dphi)-shift)+1;
        ...
        private void check_pixel (int o, int omax, int zone,
            RangeSet pixset, long pix, pstack stk, bool inclusive)
        {
            if (zone==0) return;
            ...
            else if (o>order) // this implies that inclusive==true
            {
                ...
                {
                    pixset.append((long)((ulong)pix>>(2*(o-order)))); // output the parent pixel at order
        ...
        public long pix2ring (long pix)
        {
            ...
            {
                if (pix<ncap) // North Polar cap
                    return (long)((ulong)(1+HealpixUtils.isqrt(1+2*pix))>>1); // counted from North pole
                else if (pix<(npix-ncap)) // Equatorial region
                    return (pix-ncap)/nl4 + nside; // counted from North pole
                else // South Polar cap
                    return nl4-(long)((ulong)(1+HealpixUtils.isqrt(2*(npix-pix)-1))>>1);
            }
            else
            {
                Xyf xyf = nest2xyf(pix);
        ...
        /** Returns a set of points along the boundary of the given pixel.
            ...
            * 
            */
        public Vec3[] boundaries(long pix, int step)
        {
        ...
        /** Returns a set of points along the boundary of the given pixel.
            ...
            * @param pix pixel index number
            * @param step the number of returned points is 4*step
            * @return {@link Zphi} for each point
            * 
            */
        public Zphi[] boundariesZphi(long pix, int step)
        {
            HealpixUtils.check(step>0,"step must be positive");
            Zphi[] points = new Zphi[4*step];
            Xyf xyf = pix2xyf(pix);
            double dc=0.5/nside;
            double xc=(xyf.ix+0.5)/nside, yc=(xyf.iy+0.5)/nside;
            double d = 1/(step*nside);
            for (int i=0; i<step; ++i)
            {
                points[i       ]=new Fxyf(xc+dc-i*d, yc+dc, xyf.face).toZphi();
                points[i+  step]=new Fxyf(xc-dc, yc+dc-i*d, xyf.face).toZphi();
                points[i+2*step]=new Fxyf(xc-dc+i*d, yc-dc, xyf.face).toZphi();
                points[i+3*step]=new Fxyf(xc+dc, yc-dc+i*d, xyf.face).toZphi();
            }
            return points;
        }
    }
